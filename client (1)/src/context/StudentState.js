...
const StudentState = (props) => {
  ...
  const getTotalRegistrationStatus = async (data) => {

    console.log("student context registration status=",data)

    let totalRegistrationStatus = await fetch('http://localhost:8000/totalRegistrationStatus', {
      method: 'POST',
      headers: {
        "Content-Type": "application/json"
      },
    ...
    totalRegistrationStatus = await totalRegistrationStatus.json()
    return(totalRegistrationStatus)
