...
// router.post('/delete-google-sheet', async (req, res) => {
//     try {
//         const serviceAccountAuth = new JWT({
//             email: 'registrationgooglesheet@optical-wall-409909.iam.gserviceaccount.com',
//             key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
...
//         // Assume you want to clear the values in the specified row
//         await sheet.loadCells();

//         const rows = await sheet.getRows();
//         const rowIndex = parseInt(req.body.index);

//         if (rows[rowIndex]) {
//             await rows[rowIndex].delete();
//             res.json({ "status": true });
//         } else {
//             res.json({ "status": false, "message": "Row not found" });
//         }
//     } catch (error) {
...
router.post('/delete-google-sheet', async (req, res) => {
    try {
        ...
        const rows = await sheet.getRows();
        const rowIndex = parseInt(req.body.index);
        console.log('index =',rowIndex,rows[rowIndex])
...
// delete registartion Student 


router.post("/deleteRegisterStudent", async (req, res) => {
    
    // console.log("register route =", req.body)
    try {
    
    const deleteStudentByRegistration = await totalRegistrationDev.deleteOne({"RegistrationNo":req.body.RegistrationNo})
    const deleteStudent = await registerStudentDev2.deleteOne({"RegistrationNo":req.body.RegistrationNo})
...
router.get("/getregisterStudent", async (req, res) => {
    try {
        ...
        const totalRegistration  = await totalRegistrationDev.find();
        res.status(200).json(userdata);
    } catch (error) {
        console.log('error =', error.message)
        res.status(500).json(error);
...
// totalRegistartion count


router.post("/totalRegistrationStatus", async (req, res) => {
    console.log("total registration status")
    try {
        
        const totalRegistrationCourse  = await totalRegistrationDev.find({"month":req.body.month,"year":req.body.year,"Course":req.body.Course});
        let totalCount  = totalRegistrationCourse.length>10?totalRegistrationCourse.length:`0${totalRegistrationCourse.length}`

        let courseCount = req.body.RegistrationNo.split("/")[2].split("-")[1]
